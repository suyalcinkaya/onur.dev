...
summary: 'Often we see ourselves writing almost the same exact codes in more than one different components. Ideally what we could do in such cases would be to extract that recurrent logic into a reusable piece of code (hook) and reuse it where the need be.'
publishedAt: '2020-05-17'
image: '/static/images/html5-video.webp'
categories: 'React, Hooks, JavaScript'
---

One of the most common React Hooks that I’ve found myself using is one to handle API calls. Often we see ourselves writing almost the same exact codes in more than one different components. Ideally what we could do in such cases would be to extract that recurrent logic into a reusable piece of code (hook) and reuse it where the need be.
...
Here is an another must feature for our custom hook. We’ll define 2 states, one is a boolean value that indicates whether has an error or not and another one is a string value that includes error message.
...
We’ll change `url` prop name to `initialUrl` to use it as a state. The `initialUrl` prop will be default value of the `url` state.

We’ll add `url` state to dependency of the `useEffect` hook to trigger it again. Then we’ll export its `setState` function which is `updateUrl` to use it from our component.
...
We’ll take parameters as a prop which will be an object. And then, we’ll transpile them with `encodeURIComponent` to use properly. Following from that, we’ll add `params` state to dependency of the `useEffect` hook to trigger it again. It would look like this:
