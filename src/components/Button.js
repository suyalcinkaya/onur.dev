...
export const OutlineButton = ({ as = 'button', className = '', isExternal, ...others }) => {
  ...
  return (
    <Tag
      className={`${commonClassNames} ${className} text-sm h-8 hover:bg-gray-900 px-3`}
      {...(as === 'button' && {
        type: 'button'
      })}
      {...(isExternal && {
...
export const GhostButton = ({ as = 'button', className, isExternal, ...others }) => {
  ...
  return (
    <Tag
      className={`${commonClassNames} ${className} bg-transparent px-4 h-10 focus:ring-2 focus:ring-gray-500 focus:ring-opacity-50`}
      {...(as === 'button' && {
        type: 'button'
      })}
      {...(isExternal && {
