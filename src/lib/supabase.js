...
export async function getViewCounts() {
  ...
  return data.map(({ id, slug, view_count }) => ({ id, slug, view_count }))
}
...
export async function upsertViewCount(slug) {
  ...
  if (process.env.NODE_ENV !== 'production' || !slug) return { view_count: latestData?.view_count ?? 0 }
  ...
  const { data: newOrUpdatedData } = await supabase
    ...
    .upsert({
      id: latestData?.id,
      slug: slug,
      view_count: latestData?.view_count ? latestData.view_count + 1 : 1,
      view_count_updated_at: new Date()
    })
    .select()

  return { view_count: newOrUpdatedData[0]?.view_count ?? 0 }
}
